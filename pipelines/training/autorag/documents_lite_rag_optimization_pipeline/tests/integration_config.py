"""RHOAI integration test config for Lite AutoRAG: load .env and build config from environment.

Used by conftest.py (fixtures) and test_pipeline_integration.py (skipif) so
skip logic and fixtures share one source of truth.
"""
...
# Pipeline parameters (Lite: no llama-stack; uses chat/embedding model URLs and tokens)
...
CHAT_MODEL_URL_ENV = "CHAT_MODEL_URL"
CHAT_MODEL_TOKEN_ENV = "CHAT_MODEL_TOKEN"
EMBEDDING_MODEL_URL_ENV = "EMBEDDING_MODEL_URL"
EMBEDDING_MODEL_TOKEN_ENV = "EMBEDDING_MODEL_TOKEN"
...
def get_docrag_lite_integration_config():
    ...
    chat_url = os.environ.get(CHAT_MODEL_URL_ENV)
    chat_token = os.environ.get(CHAT_MODEL_TOKEN_ENV)
    emb_url = os.environ.get(EMBEDDING_MODEL_URL_ENV)
    ...
    return {
        ...
        "rhoai_project": (project or "docrag-lite-integration-test").strip(),
        ...
        "chat_model_url": chat_url.strip(),
        "chat_model_token": chat_token.strip(),
        "embedding_model_url": emb_url.strip(),
        "embedding_model_token": emb_token.strip(),
...
DOCRAG_LITE_INTEGRATION_CONFIG = get_docrag_lite_integration_config()
