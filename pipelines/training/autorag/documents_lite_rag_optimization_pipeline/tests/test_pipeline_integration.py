"""High-level integration tests for Documents Lite RAG Optimization pipeline on RHOAI.
...
parameters (secret names, bucket names, keys, chat/embedding model URLs and tokens).
See conftest.py and integration_config.py. When not set, tests are skipped.
Set vars via a .env file (see .env.example).
...
def _make_docrag_lite_run_name():
    """Return a run name: docrag-lite-test-<6 hex chars>-<YYYYMMDD-HHMMSS>."""
    hex_part = secrets.token_hex(3)
    time_part = datetime.now(timezone.utc).strftime("%Y%m%d-%H%M%S")
    return f"docrag-lite-test-{hex_part}-{time_part}"
...
def _run_pipeline_and_wait(client, compiled_path, arguments, timeout):
    ...
    run_name = _make_docrag_lite_run_name()
...
def _find_artifacts_in_s3(s3_client, bucket, prefix):
    ...
    rag_patterns, and .ipynb notebooks.
    """
    html_keys = []
    ipynb_keys = []
    pattern_keys = []
...
def _pipeline_arguments_from_config(config):
    """Build pipeline arguments dict from integration config (Lite: chat/embedding URLs and tokens)."""
    return {
        ...
        "chat_model_token": config["chat_model_token"],
        "embedding_model_url": config["embedding_model_url"],
        "embedding_model_token": config["embedding_model_token"],
    }
...
class TestDocumentsLiteRagOptimizationPipelineIntegration:
    """Integration tests running the Lite pipeline on RHOAI and validating outcomes."""

    def test_documents_lite_rag_optimization_pipeline_run(
        self,
        ...
        """Run Lite pipeline; assert success and optional presence of artifacts in S3."""
        if not kfp_client:
            pytest.skip("Integration prerequisites not available")
        config = docrag_lite_integration_config
