"""Pytest fixtures for Documents Lite RAG Optimization pipeline tests."""
...
@pytest.fixture(scope="session")
def kfp_client(docrag_lite_integration_config):
    """Session-scoped KFP client pointing to RHOAI pipeline API."""
    if docrag_lite_integration_config is None:
        return None
    import kfp

    host = docrag_lite_integration_config["rhoai_kfp_url"]
    ...
    return kfp.Client(
        host=host,
        namespace=docrag_lite_integration_config["rhoai_project"],
        existing_token=docrag_lite_integration_config.get("rhoai_token"),
...
def _sanitize_pipeline_yaml_to_ascii(path: Path) -> None:
    ...
    try:
        ...
        return
    sanitized = "".join(c if ord(c) < 128 else " " for c in text)
    if sanitized != text:
...
def compiled_pipeline_path():
    """Compile the Documents Lite RAG Optimization pipeline to a temp YAML file."""
    ...
    compiler.Compiler().compile(
        pipeline_func=documents_lite_rag_optimization_pipeline,
        package_path=path,
    )
    _sanitize_pipeline_yaml_to_ascii(path_obj)
    yield path
...
def s3_client(docrag_lite_integration_config):
    ...
    try:
        ...
        return None
    c = docrag_lite_integration_config
    return boto3.client(
        "s3",
